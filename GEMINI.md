...
- Al analizar el repositorio, ten en cuenta que el paquete `play-ondas-app-design/` es el contrato visual vinculante para v1. Para cualquier cambio de UI, aplica el checklist completo de `AGENTS.md §23`: tokens, colores de onda restringidos, tipografía (Hanken Grotesk UI / Space Mono datos), iconos Lucide, tamaño de ventana 900×620 y mínimo 720×560, comparación con las 10 capturas de referencia y verificación de accesibilidad.
- El proyecto usa Rust para la capa Tauri. Revisa `Cargo.toml` y la constitución antes de proponer cambios en `src-tauri/`.
- La constitución tiene versionado explícito (v1.5.0). Si propones una enmienda, sigue el proceso descrito en la sección 20 de `AGENTS.md` y usa la skill `speckit-constitution` si está disponible.
- El sistema de logging tiene requisitos estrictos de formato horario: zona `Europe/Madrid` con formato `DD/MM/YYYY HH:mm:ss` en logs legibles. No propongas formatos alternativos sin enmienda constitucional.
